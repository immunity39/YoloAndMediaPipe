...
MARKER_SEPARATION = 0.01          # 1cm間隔（必要に応じて変更）
...
def build_board(dictionary, marker_length, marker_sep):
    """バージョンに応じてGridBoardを生成"""
    board = None
    try:
        # 新API（OpenCV ≥4.7）
        board = aruco.GridBoard.create(2, 2, marker_length, marker_sep, dictionary)
...
def main():
    ...
    board = build_board(ar_dict, BOARD_MARKER_LENGTH, MARKER_SEPARATION)
    ...
    while True:
        ...
        corners, ids, _ = aruco.detectMarkers(gray, ar_dict, parameters=detector_params)
        plane_ok = False
            ...
            for handLms in results.multi_hand_landmarks:
                ...
                if plane_ok:
                    ...
                    if p_int is not None:
                        ...
                        cv2.putText(frame, f"{dist_m*1000:.1f} mm", (10, 30),
